...
class User(Base):
    ...
    meters = relationship(
        ...
        foreign_keys="Meter.owner_id"
    )
    assigned_meters = relationship(
        "Meter",
        back_populates="assigned_user",
        foreign_keys="Meter.assigned_to"
    )
...
class Meter(Base):
    ...
    owner = relationship("User", back_populates="meters", foreign_keys=[owner_id])
    assigned_to = Column(Integer, ForeignKey("users.id"), nullable=True)
    assigned_user = relationship("User", back_populates="assigned_meters", foreign_keys=[assigned_to])
    created_at = Column(DateTime(timezone=True), server_default=func.now())
